import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import * as jobService from '../../services/jobs/job.service.js';
import useSocket from '../../hooks/useSocket.js';
import useBidStore from '../../store/bids/bidStore.js';
import CountdownTimer from '../../components/bidding/CountdownTimer.jsx';
import LiveBidCounter from '../../components/bidding/LiveBidCounter.jsx';
import BidList from '../../components/bidding/BidList.jsx';
import Spinner from '../../components/common/Spinner.jsx';
import toast from 'react-hot-toast';
import { MapPin, AlertCircle } from 'lucide-react';

export const JobBidding = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const socket = useSocket();
  const { bidCount } = useBidStore();

  const [job, setJob] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        const response = await jobService.getJobById(id);
        setJob(response.data.data);
      } catch (err) {
        console.error(err);
        setError('Failed to load job details.');
      } finally {
        setLoading(false);
      }
    };
    fetchJobDetails();
  }, [id]);

  // Listen for real-time acceptance or cancellation from WebSocket
  useEffect(() => {
    if (!socket || !id) return;

    const handleBidAccepted = ({ jobId, bidId, bookingId }) => {
      if (jobId === id) {
        toast.success('Bid accepted successfully!');
        setTimeout(() => {
          navigate(`/customer/bookings/${bookingId}`);
        }, 1500);
      }
    };

    const handleJobCancelled = ({ jobId }) => {
      if (jobId === id) {
        toast.error('This job has been cancelled.');
        navigate('/customer/dashboard');
      }
    };

    socket.on('bid:accepted', handleBidAccepted);
    socket.on('job:cancelled', handleJobCancelled);

    return () => {
      socket.off('bid:accepted', handleBidAccepted);
      socket.off('job:cancelled', handleJobCancelled);
    };
  }, [socket, id, navigate]);

  const handleCancelJob = async () => {
    if (window.confirm('Are you sure you want to cancel this job?')) {
      try {
        await jobService.cancelJob(id);
        toast.success('Job cancelled successfully.');
        navigate('/customer/dashboard');
      } catch (err) {
        toast.error(err.response?.data?.message || 'Failed to cancel job.');
      }
    }
  };

  if (loading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center flex-col gap-4">
        <Spinner size="lg" />
        <p className="text-sm font-semibold text-gray-500">Loading job details...</p>
      </div>
    );
  }

  if (error || !job) {
    return (
      <div className="max-w-md mx-auto mt-12 p-6 bg-white border border-gray-200 rounded-xl shadow-sm text-center">
        <AlertCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
        <h3 className="text-lg font-bold text-gray-900">Error Loading Job</h3>
        <p className="text-sm text-gray-500 mt-1">{error || 'Job not found.'}</p>
        <button
          onClick={() => navigate('/customer/dashboard')}
          className="mt-5 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg text-sm transition"
        >
          Back to Dashboard
        </button>
      </div>
    );
  }

  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      {/* Upper Status Panel */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-800 border border-green-200 capitalize">
                {job.service}
              </span>
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-200 capitalize">
                {job.urgency} Urgency
              </span>
            </div>
            <h1 className="text-2xl font-black text-gray-900 mt-2 tracking-tight">Live Bidding Room</h1>
            <p className="text-sm text-gray-500 mt-1">Job ID: {job._id}</p>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <CountdownTimer expiresAt={job.expiresAt} />
            <LiveBidCounter count={bidCount} />
          </div>
        </div>

        {/* Job details box */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-100">
          <div className="md:col-span-2 space-y-4">
            <div>
              <h3 className="text-xs font-semibold text-gray-400 tracking-wider uppercase">Job Description</h3>
              <p className="text-sm text-gray-800 mt-1 leading-relaxed">{job.description}</p>
            </div>
            {job.address && (
              <div className="flex items-start gap-1.5 text-xs text-gray-500">
                <MapPin className="h-4 w-4 text-gray-400 shrink-0 mt-0.5" />
                <span>{job.address.full || `${job.address.city}, ${job.address.pincode}`}</span>
              </div>
            )}
            {job.imageUrl && (() => {
              const isCloudinary = job.imageUrl.startsWith('http');
              const baseUrl = import.meta.env.VITE_API_BASE_URL?.replace('/api', '') || 'http://localhost:5000';
              const cleanPath = job.imageUrl.replace(/\\/g, '/').replace(/^\/+/, '');
              const fullUrl = isCloudinary ? job.imageUrl : `${baseUrl}/${cleanPath}`;
              
              return (
                <div className="mt-4 space-y-2">
                  <h3 className="text-xs font-semibold text-gray-400 tracking-wider uppercase">Reference Image</h3>
                  <div className="rounded-xl overflow-hidden border border-gray-200 w-full md:w-2/3 lg:w-1/2 relative group">
                    <img src={fullUrl} alt="Job reference" className="w-full h-auto object-cover" />
                    <a href={fullUrl} target="_blank" rel="noopener noreferrer" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white font-bold text-sm">
                      View Full Image
                    </a>
                  </div>
                </div>
              );
            })()}
          </div>

          <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex flex-col justify-between">
            <div>
              <span className="text-xs font-semibold text-gray-400 tracking-wider uppercase">Max Budget</span>
              <p className="text-3xl font-black text-gray-900 mt-1">₹{job.budget}</p>
            </div>
            {job.status === 'open' && (
              <button
                onClick={handleCancelJob}
                className="mt-4 w-full py-2 bg-white hover:bg-red-50 text-red-600 border border-red-200 hover:border-red-300 font-bold rounded-lg text-xs transition"
              >
                Cancel Service Request
              </button>
            )}
          </div>
        </div>
      </div>

      {/* Bids Section */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
        <h2 className="text-lg font-bold text-gray-950 mb-4">Received Bids</h2>
        <BidList jobId={id} isCustomer={true} />
      </div>
    </div>
  );
};

export default JobBidding;
