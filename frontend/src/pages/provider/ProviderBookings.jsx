...
import { CheckCircle, Clock, XCircle, MapPin, ChevronRight } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const handleUpdate = (data) => {
      if (data.bookingId && data.status) {
        setBookings(
          bookings.map(b => b._id === data.bookingId ? { ...b, status: data.status } : b),
          useBookingStore.getState().pagination
        );
        // Refresh full list if tab is specific
        if (activeTab !== 'all') fetchBookings();
      }
    };
    socket.on('booking:updated', handleUpdate);
    return () => socket.off('booking:updated', handleUpdate);
  }, [socket, bookings, activeTab]);
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">View all customer bookings assigned to you.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {bookings.map((booking) => {
            ...
            const addr = booking.jobId?.address;
            ...
            return (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                    <span className="font-semibold">
                      Customer: <span className="text-gray-700">{booking.customerId?.name || '—'}</span>
                    </span>
                    <span>•</span>
                    <span>
                ...
                <div className="flex flex-col items-end shrink-0">
                  <p className="text-xl font-black text-green-700">₹{booking.basePrice}</p>
                  <p className="text-xs text-gray-400 font-semibold mt-0.5">Your Earnings</p>
